...
const translationService = (function () {
  ...
  // --- TWP-FullPage: job-scoped service-worker keepalive --------------------
  // MV3 evicts the background service worker after ~30s idle. On a long chapter
  // that can happen mid-batch, dropping in-flight requests (recoverable via
  // retry/re-sweep, but it stalls). This keepalive pings a cheap extension API
  // on an interval ONLY while translation work is actually in flight (refcounted
  // acquire/release), and stops the moment the page is idle — so there's no
  // permanent wakelock / battery cost when you're just reading.
  const swKeepAlive = (function () {
    let refs = 0;
    let timer = null;
    const ping = () => {
      try {
  ...
  class Service {
      ...
      for (const sourceArray of sourceArray2d) {
        ...
        if (progressInfo) {
          ...
          if (cacheEntry) {
            ...
            // TWP-FullPage patch: 800 -> 1100. Larger batches give the
            // translator more surrounding context (better accuracy) and cut
            // the number of round-trips (faster), while staying well under the
            // endpoint's limits.
            if (currentSize > 1100) {
    ...
    async makeRequest(sourceLanguage, targetLanguage, requests) {
      // TWP-FullPage patch: retry with backoff. Full-page mode fires many
      // parallel requests, so transient failures (network blips, Google
      // rate-limiting with HTTP 429, timeouts) are more likely. Without retry
      // those chunks stay permanently untranslated until a manual re-run.
      // Up to 3 attempts with 400ms / 1200ms backoff.
      const maxAttempts = 3;
      let lastError;
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        if (attempt > 0) {
          let delay = attempt === 1 ? 400 : 1200;
          // honor the server's Retry-After (capped at 6s) when the last
          // failure carried one — waiting the asked amount clears 429s faster
          // than blindly retrying into the same rate limit
          if (lastError && lastError.retryAfterMs > delay)
            delay = lastError.retryAfterMs;
          await new Promise((r) => setTimeout(r, delay));
        }
        try {
          return await this.makeRequestOnce(
            sourceLanguage,
            targetLanguage,
            requests
          );
    ...
    async makeRequestOnce(sourceLanguage, targetLanguage, requests) {
      return await new Promise((resolve, reject) => {
        ...
        // TWP-FullPage patch: bound each request so a hung connection doesn't
        // stall a chunk forever — it fails and the retry/backoff wrapper takes
        // over. 20s is generous for a translation batch.
        try {
          xhr.timeout = 20000;
        } catch (e) {}

        xhr.onload = (event) => {
          // TWP-FullPage patch: treat HTTP error statuses (429 rate-limit,
          // 5xx server errors) as failures so the retry/backoff wrapper
          // engages instead of passing a bad body downstream.
          const status = xhr.status;
          if (status && (status === 429 || status >= 500)) {
            const err = new Error("HTTP " + status);
            // carry Retry-After (seconds) so the retry wrapper can honor it
            try {
    ...
    async translate(
      ...
      // TWP-FullPage patch: cap concurrent in-flight requests. Firing every
      // chunk at once (full-page mode can produce dozens) tends to trip
      // Google's rate limiter (HTTP 429), which then forces retries and makes
      // the whole page slower and less reliable. A small concurrency window
      // keeps requests flowing steadily without the 429 storms.
      ...
      function handleRequest(request) {
        return self
          ...
          .then((response) => {
            const results = self.cbParseResponse(response);
            for (const idx in request) {
              const result = results[idx];
              self.cbTransformResponse(result.text, dontSortResults);
