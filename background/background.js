...
if (typeof chrome.contextMenus !== "undefined") {
  const updateActionContextMenu = () => {
    ...
    // TWP-FullPage patch: "page_action" is an MV2-era context. MV3 has no
    // pageAction (it merged into "action"), so these were always dead weight
    // in Chrome — but Safari's stricter validator REJECTS "page_action"
    // outright and throws, which kills the whole background script and can
    // cascade into "duplicate variable" errors on reload. Dropped entirely;
    // "action" alone covers everything MV3 needs, on both browsers.
    chrome.contextMenus.create({
