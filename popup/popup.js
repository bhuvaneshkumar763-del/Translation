  ...
  .then(() => {
    ...
    chrome.tabs.query(
      ...
      (tabs) => {
        ...
        // Floating bubble — per-host show/hide
        {
          const host = (() => { try { return new URL(tabs[0].url).hostname; } catch (e) { return ""; } })();
          const map = twpConfig.get("fpBubbleByHost") || {};
          $("#cbShowFloatingBubble").checked =
            Object.prototype.hasOwnProperty.call(map, host)
              ? map[host] !== "no"
              : twpConfig.get("fpShowFloatingBubble") !== "no";
          $("#cbShowFloatingBubble").addEventListener("change", (e) => {
            const m = Object.assign({}, twpConfig.get("fpBubbleByHost") || {});
            if (host) m[host] = e.target.checked ? "yes" : "no";
            twpConfig.set("fpBubbleByHost", m);
          });
        }
