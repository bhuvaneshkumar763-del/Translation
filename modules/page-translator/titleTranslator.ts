...
/**
 * Tab-bar title translation, ported from the fork's
 * `contentScript/pageTranslator.js` (`translateTitleString`, `applyTabTitle`,
 * `translatePageTitle`, `maybeRetranslateTitle`, `setupTitleObserver`,
 * `teardownTitleObserver`). Kept as its own module (not folded into
 * `translateLoop.ts`) so the cache, the batching workaround, and the
 * dual-write are unit-testable without a DOM-walking page.
 *
 * Why this needs a workaround at all: `modules/providers/google.ts`'s
 * `cbTransformRequest` only wraps a request in `<a i=N>` markers when the
 * batch has more than one item (`arr.length > 1`) — Google's endpoint does
 * not reliably translate a single bare string sent without that wrapper.
 * `collectTextNodes` in `translateLoop.ts` never reaches `<title>` (it walks
 * from `document.body`, and `<title>` lives in `<head>`), so page
 * translation alone never touches the tab title, and a naive one-off
 * single-string translation call for the title would hit the exact
 * `arr.length > 1` gap above and silently come back untranslated.
 *
 * The fork tried fixing this by making the `<a i=N>` wrapping unconditional
 * everywhere — that's a global change hitting every single-text translation
 * path in the extension (selection popup, hover tooltips, ...), not just the
 * title, and it made some of those worse. Do not repeat that here. Instead,
 * scope the fix to just this module: send the title alongside a throwaway
 * second string (`[[title, ' ']]`), which reliably lands on the >1 branch,
 * then keep only the first result and discard the second.
 */
...
export function createTitleTranslator(options: TitleTranslatorOptions) {
  ...
  async function translateTitleString(text: string): Promise<string | null> {
    ...
    try {
      let results: string[][];
      try {
        results = await sendMessage('translateHTML', {
  ...
  function startWatching(): void {
    ...
    // visibilitychange handling below does one catch-up check on refocus.
