...
export default defineContentScript({
  // Back to the default 'manifest' registration with a static `matches`
  // — reverted from Gen 2 Session 4's activeTab-gated `registration:
  // 'runtime'` experiment. See CLAUDE.md's "Permission model: reverted to
  // unconditional access" section for the full account of why: automatic
  // "always translate" only ever worked on browsers supporting the
  // dynamic scripting.registerContentScripts API that setup depended on,
  // which a real user's browser (Orion, WebKit-based) doesn't — confirmed
  // against their own previously-working pre-rewrite fork, which used
  // exactly this static-matches shape with unconditional host_permissions
  // and had no such gap.
  matches: ['<all_urls>'],
  runAt: 'document_end',
  allFrames: true,
  ...
  cssInjectionMode: 'ui',
  async main(ctx) {
    // Guards against double-running everything below (duplicate mutation
    // observers, duplicate onMessage registrations — @webext-core/messaging
    // itself throws on "only one listener can be setup for X"). Mainly
    // matters for a tab that was already open *before* this extension was
    // installed/reloaded: its static content_scripts injection never ran,
    // so ensureContentScript.ts's on-demand executeScript fallback injects
    // it instead the first time that tab is used — and if the user
    // triggers that fallback twice in quick succession before the first
    // injection's guard flag lands, both injected copies would otherwise
    // run main() concurrently.
